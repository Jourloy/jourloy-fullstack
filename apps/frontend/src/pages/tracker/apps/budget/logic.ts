import {TBudget, TSpend} from "src/store/features/budget.slice";
import TrackerAPI from "./budgetAPI";
...
export default class BudgetLogic {
	...
	public getDays() {
		...
		return Math.floor(this.budget.limit / this.budget.periodLimit);
	}
	...
	public getTotalIncome() {
		...
		const spends = this.budget.spends.filter(s => !s.date && s.cost > 0);
	...
	public getDaysCount() {
		const startDate = new Date(this.budget.startDate).getTime();
		const today = Date.now();
		const days = Math.floor((today - startDate) / 1000 / 60 / 60 / 24);

		const startBudget = this.budget.limit;
	...
	public getTodayLimit() {
		...
		let startDate = new Date(this.budget.startDate).getTime();
		if (new Date(this.budget.startDate).getHours() > 0) {
			startDate -= 1000 * 60 * 60 * new Date(this.budget.startDate).getHours();
		}
		if (new Date(this.budget.startDate).getMinutes() > 0) {
			startDate -= 1000 * 60 * new Date(this.budget.startDate).getMinutes();
		}
		if (new Date(this.budget.startDate).getSeconds() > 0) {
			startDate -= 1000 * new Date(this.budget.startDate).getSeconds();
		}
		...
		return this.budget.periodLimit * days + sums;
	}
